...
class Worker:
    ...
    def check_certificate_expiry(self, cert):
        ...
        if days_until_expiry < 0:
            print(
                f"[⚠️] WARNING: Certificate EXPIRED "
                f"{abs(days_until_expiry)} days ago!"
            )
        elif days_until_expiry < 30:
            print(
                f"[⚠️] WARNING: Certificate expires in "
                f"{days_until_expiry} days!"
            )
        else:
            print(
                f"[✓] Certificate valid for "
                f"{days_until_expiry} more days"
            )
    ...
    def work(self):
        ...
        print(
            "[*] Supporting 9 job types: factorial, fibonacci, sum, "
            "sleep, prime, power, gcd, sort, matrix"
        )
